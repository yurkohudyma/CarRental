...
<html>
<head>
    <fmt:message key="order.warning_header" var="decline_header"/>
    <title>${decline_header}</title>
        ...
        <c:choose>
            <c:when test="${not empty order_id}">
                <form action="decline_warning" method="post">
                    <fmt:message key="order.decline.confirm_info" var="confirm_info"/>
                    <fmt:message key="order.remove.confirm" var="confirm_decline"/>
                        ${confirm_info}:
                    <input type="submit" value="${confirm_decline}">
                    <input type="hidden" name="actionName" value="confirmed">
                        <%--<input type="hidden" name="order_id" value="${order_id}">--%>

                </form>
                <c:if test="${not empty error}">
                    <fmt:message key="order.decline.error_msg" var="decline_error"/>
                    ${decline_error}
                </c:if>
            </c:when>
            <c:otherwise>
